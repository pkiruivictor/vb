...
# New function to send HTTP request and check response 
def check_http(ip, timeout):
    url = f"https://{ip}"
    try:
        response = requests.get(url, timeout=timeout, verify=False)
        if response.status_code == 200:
            print(colored(f"{ip} is alive", "green"))
            print(colored(f"Server: {response.headers.get('Server')}", "green"))
            with open(output_file, "a") as f:
                f.write(f"{ip},{response.headers.get('Server')}\n")
        else:
            print(colored(f"{ip} is not reachable", "red"))
    except requests.exceptions.ConnectionError:
        print(colored(f"{ip} is not reachable", "red"))
    except requests.exceptions.Timeout:
        print(colored(f"Timeout occured for {ip}", "yellow"))
...
# Function to process an IP address using Python's ssl library
# Update process_ip function to use new check_http function
def process_ip(ip, timeout, output_file):
    check_http(ip, timeout)
